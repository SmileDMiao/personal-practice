...
class ChatChannel < ApplicationCable::Channel
  ...
  def speak(data)
    ...
    ActionCable.server.broadcast "chat_channel#{current_user.id}", message: render_message_from(message)
  ...
  def render_message_from(message)
    ApplicationController.renderer.render(partial: "chats/chat", locals: {message: message, current_user: current_user})
  end
